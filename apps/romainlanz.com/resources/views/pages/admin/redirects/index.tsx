	...
	return (
		<Admin title="Redirections">
			<div class="d-flex column" style={{ gap: space(5) }}>
				<div>
					<Button href={route('admin.redirects.create')} up-layer="new" up-mode="modal" up-target="[up-modal-scope]">
						Ajouter
					</Button>
				</div>
				...
				<Table headers={['URL', 'À', 'Visites', '']}>
					{redirects.map((redirect) => (
						<tr>
							...
							<td>
								<form action={route('admin.redirects.delete', [redirect.id]) + '?_method=DELETE'} method="post">
									{csrfField()}
